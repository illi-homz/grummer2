...
grummer.ourworks = {
  ...
  initSlider() {
    ...
    this.slider.slick(slickParams); // this.slider.on("afterChange", () => this.moveArrows(false));
...
grummer.promo = {
  ...
  createSliderItem(el) {
    ...
    img.style.setProperty("--m-image", `url(../img/promo/mobile/${el.img})`, "");
    img.style.setProperty("--t-image", `url(../img/promo/tablet/${el.img})`, "");
    item.append(img);
    return item;
  }
...
grummer.init = function () {
  ...
  this.questions.init();
  this.feedbacks.init();
  this.promo.init(); // grummer.popupMain.init()
};
