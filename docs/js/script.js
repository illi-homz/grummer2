...
grummer.gSelect = {
  ...
  setInputValue: function setInputValue($select, value) {
...
grummer.header = {
  ...
  changeBg: function changeBg() {
    ...
    var timeFade = 1000;
    var timeChange = 9000;
    var arrLen = this.backgrounds.length;
    return setInterval(function () {
      if (count > arrLen - 1) count = 0;

      _this.headerBg.fadeOut(timeFade, function () {
        _this.headerBg.css({
          background: "url(../img/main/".concat(_this.backgrounds[count], ") bottom / cover no-repeat")
        });

        _this.headerBg.fadeIn(timeFade);

        count++;
      });
    }, timeChange);
  },
  linksTemplating: function linksTemplating(list) {
    var _this2 = this;

    var isMobile = list.hasClass('header__mobile-menu-links');
    var frag = '';
    $.each(this.links, function (index, obj) {
      frag += _this2.linksTemplate.replace(/{{title}}/ig, obj.title).replace(/{{value}}/ig, obj.value).replace(/{{isMobile}}/ig, isMobile);
    });
    list.append(frag);
  },
...
grummer.services = {
  ...
  setBreeds: function setBreeds(arr) {
    var list = $('.services__breeds .g-select__items');
    var template = $.trim($('.services__breeds .services__filter-temp').html());
    var frag = '';
    arr.forEach(function (item) {
      frag += template.replace(/{{title}}/ig, item.title).replace(/{{value}}/ig, item.value);
    });
    list.append(frag);
  },
  setCategories: function setCategories(arr) {
    var list = $('.services__categories .g-select__items');
    var template = $.trim($('.services__categories .services__filter-temp').html());
    var frag = '';
    arr.forEach(function (item) {
  ...
  setSlides: function setSlides(slides, list, template) {
    list.html('');
    var frag = '';
  ...
  setBreed: function setBreed(val) {
    this.breed = val;
  },
  ...
  filterServices: function filterServices(val, type) {
    return grummer.servicesList.filter(function (service) {
      return service[type] === val;
    });
  },
  filterServicesByBreed: function filterServicesByBreed(el) {
    var animal = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
    var $el = $(el);
    if ($el.hasClass('active')) return;
    $el.parent().children('div').removeClass('active');
    $el.addClass('active');
    var dogsMenu = $('.services__info-for-dogs');
  ...
  filterServicesByCategory: function filterServicesByCategory(val) {
