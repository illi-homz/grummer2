...
grummer.popupMain = {
  ...
  createServicesListHtml() {
    ...
    return grummer.currentServices.reduce((acc, service) => {
      return acc += template.replace(/{{title}}/ig, service.title).replace(/{{price}}/ig, service.price);
    }, '');
  },
...
grummer.questions = {
  init() {
    ...
    this.items = this.$spoiler.find('.questions__item'); // this.items.first().find('.questions__item-text')
  },
